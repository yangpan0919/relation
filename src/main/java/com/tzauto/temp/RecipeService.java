...
public class RecipeService extends BaseService {
    ...
    public RecipeService(SqlSession sqlSession) {
        ...
        recipeNameMappingMapper = this.session.getMapper(RecipeNameMappingMapper.class);
    }
    ...
    public String downLoadRcp2DeviceByType(DeviceInfo deviceInfo, Recipe recipe, String eventId, String type) {
        ...
        if(deviceInfo.getDeviceType().equals("PLASMA")){
            PlasmaHost equipModel = (PlasmaHost)GlobalConstants.stage.equipModels.get(deviceInfo.getDeviceCode());
            list = (ArrayList<String>) equipModel.getEquipRecipeList().get("eppd");
            if (list.size() == 1 && list.get(0).equals(recipeName)) {
                logger.info("已有程序名，与所需要选择的一致");
                return "0";
            }else if (list.size() > 1) {
                logger.error("机台recipe程序有多个，请删除多余的recipe再操作" + list.size());
                return "机台recipe程序有多个，请删除多余的recipe再操作";
            }else{
                equipModel.setNoSelectRecipe(false);
            }
        }
        ...
        if (type.contains("Select")) {
            ...
            if (!ppselectResult.equals("0") && (!deviceInfo.getDeviceType().equals("PLASMA"))) {
        ...
        if (type.contains("DeleteAll")) {
            if (result.equals("0") && (deviceInfo.getDeviceType().equals("PLASMA"))) {
                EquipModel equipModel = GlobalConstants.stage.equipModels.get(deviceInfo.getDeviceCode());
                for (String s : list) {
                    if (!s.equals(recipeName)) {
                        equipModel.deleteAllRcpFromDevice(s);//删除不是选中的recipe，正常情况机台最多只有两个recipe
                    }
                }
            } else {
                String delAllResult = hostManager.deleteAllRcpFromDevice(deviceId, recipeName);
...
//        }   
    ...
    public String downLoadRcp2ISECSDeviceByTypeAutomatic(DeviceInfo deviceInfo, Recipe recipe, String type) {
        ...
        if (equipModel.getPassport()) {
            ...
            if (equipModel.deviceType.equals("PLASMA")) {
                PlasmaHost plasmaHost = (PlasmaHost) equipModel;
                String equipStatus = plasmaHost.getEquipStatus();
                if("Run".equals(equipStatus)){
                    return "设备正在运行，不能下载程序";
                }
                list = (ArrayList<String>) equipModel.getEquipRecipeList().get("eppd");
                if (list.size() == 1 && list.get(0).equals(recipeName)) {
                    flag = false;
                    plasmaHost.setNoSelectRecipe(true);
                    plasmaHost.selectRecipe(recipeName);
                    result = "0";
                    logger.info("已有程序名，与所需要选择的一致");
                } else if (list.size() > 1) {
                    logger.error("机台recipe程序大于一个，无法明确选择哪个recipe；" + list.size());
                    return "2";
                } else {
                    plasmaHost.setNoSelectRecipe(false);
                }
            }
            if (type.contains("Delete")) {//plasma 不错删除recipe操作，机台程序选中的recipe删除会报警
            ...
            if (flag) {
                ...
                if (type.contains("Select")) {
                    ...
                    logger.info("选择结果为：" + ppselectResult);

                    if (!ppselectResult.equals("0") && (!equipModel.deviceType.equals("PLASMA"))) {
                        result = result + " " + ppselectResult;
                        if (type.contains("DeleteAll")) {
                ...
                if (type.contains("DeleteAll")) {
                    if (result.equals("0") && (equipModel.deviceType.equals("PLASMA"))) {
                        for (String s : list) {
                            if (!s.equals(recipeName)) {
                                equipModel.deleteAllRcpFromDevice(s);//删除不是选中的recipe，正常情况机台最多只有两个recipe
                            }
    ...
    public String queryRecipeName( String materialNumber, String fixtureno) {
        Map paraMap = new HashMap();
